...
def harrison_ford
  ...
  Movie.select(:id, :title).joins(:actors).where("actors.name = ? AND castings.ord != ?", 'Harrison Ford', '1')
end
...
def biggest_cast
  ...
  Movie.select(:id, :title).joins(:actors).order('COUNT(actors.id) DESC').group('movies.id').limit(3)
end
